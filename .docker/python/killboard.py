...
def process_kill(killmail_data):
    ...
    try:
        ...
        final_blow_attacker = next(
            (a for a in attackers if a.get("final_blow") == True), None
        )

        # Parse the killmail_time
        killmail_time = datetime.strptime(
            esi_data.get("killmail_time", ""), "%Y-%m-%dT%H:%M:%SZ"
        )
        ...
        killmail_obj = Killmail(
            ...
            attacker_id=final_blow_attacker.get("character_id")
            if final_blow_attacker
            else None,
            attacker_corp=final_blow_attacker.get("corporation_id")
            if final_blow_attacker
            else None,
            attacker_alliance=final_blow_attacker.get("alliance_id")
            if final_blow_attacker
            else None,
            attacker_faction=final_blow_attacker.get("faction_id")
            if final_blow_attacker
            else None,
            attacker_ship=final_blow_attacker.get("ship_type_id")
            if final_blow_attacker
            else None,
        ...
        session.commit()
        logging.debug(
            f"Processed killmail sequence_id: {sequence_id}, killmail_id: {killmail_id}"
        )
...
def main():
    ...
    if last_sequence is not None:
        url2 = f"https://r2z2.zkillboard.com/ephemeral/{last_sequence}.json"
        try:
            response2 = requests.get(url2)
            if response2.status_code == 200:
                sequence = last_sequence + 1
                logging.info(
                    f"Found last sequence in DB: {last_sequence}, starting from {sequence}"
                )
            else:
                ...
                if sequence is None:
                    ...
                    return
                logging.info(f"Last sequence in DB was invalid, starting from current sequence: {sequence}")
        except requests.RequestException as e:
            logging.error(f"Failed to verify last sequence: {e}, getting current sequence")
            sequence = get_sequence()
            if sequence is None:
                logging.error("Failed to get starting sequence. Exiting.")
                return
            logging.info(f"Starting from current sequence after error: {sequence}")
    else:
        # No data in DB, start from current sequence
        sequence = get_sequence()
        if sequence is None:
            logging.error("Failed to get starting sequence. Exiting.")
            return
    ...
    while True:
        ...
        if killmail_data:
            ...
            logging.debug(f"No killmail at sequence {sequence}, waiting 10 seconds...")
            time.sleep(10)
